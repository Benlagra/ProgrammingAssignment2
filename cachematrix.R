...
## keep the information about the matrix argument x and its inverse. These functions are:
##
...
makeCacheMatrix <- function(xm = matrix()) {
       ...
       set <- function(ym) {
                     xm <<- ym
                     ixm <<- NULL        ## Inverse value stored in Extended Matrix object reset to NULL
...
## only if this has not already beeen done earlier. It acts on the extended object defined in the 
## function above and sets the value of the inverse matrix in that object if this was not done 
## before.

cacheSolve <- function(x, ...) {
        
       # Calls the value of the inverse matrix cached in the extended Matrix object x.
       ix <- x$getinverse()
       
       # If this is different from NULL, the inverse matrix has already been calculated 
       # and we can use its cached value.
       if(!is.null(ix)) {
